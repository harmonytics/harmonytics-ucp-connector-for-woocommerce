...
class UCP_WC_Cart {
	...
	public function create_cart( $metadata = null ) {
		...
		if ( false === $result ) {
			return new WP_Error(
				...
				__( 'Failed to create cart.', 'ucp-for-woocommerce' ),
				array( 'status' => 500 )
			);
		}
	...
	public function get_cart( $cart_id ) {
		...
		if ( ! $cart ) {
			return new WP_Error(
				'cart_not_found',
				__( 'Cart not found.', 'ucp-for-woocommerce' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( strtotime( $cart['expires_at'] ) < time() ) {
			return new WP_Error(
				'cart_expired',
				__( 'Cart has expired.', 'ucp-for-woocommerce' ),
				array( 'status' => 410 )
			);
		}
		...
		if ( 'converted' === $cart['status'] ) {
			return new WP_Error(
				'cart_converted',
				__( 'Cart has been converted to a checkout session.', 'ucp-for-woocommerce' ),
				array(
					'status'     => 410,
		...
		if ( count( $items ) >= self::MAX_ITEMS ) {
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Cart cannot contain more than %d items.', 'ucp-for-woocommerce' ),
					self::MAX_ITEMS
				),
				array( 'status' => 400 )
			);
	...
	public function update_item( $cart_id, $item_key, $quantity ) {
		...
		if ( false === $item_index ) {
			return new WP_Error(
				'item_not_found',
				__( 'Item not found in cart.', 'ucp-for-woocommerce' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( ! $product ) {
			return new WP_Error(
				'product_not_found',
				__( 'Product no longer exists.', 'ucp-for-woocommerce' ),
				array( 'status' => 404 )
			);
		}
	...
	public function delete_cart( $cart_id ) {
		...
		if ( false === $result ) {
			return new WP_Error(
				'cart_delete_failed',
				__( 'Failed to delete cart.', 'ucp-for-woocommerce' ),
				array( 'status' => 500 )
			);
		}
		...
		return array(
			'cart_id' => $cart_id,
			'deleted' => true,
			'message' => __( 'Cart has been deleted.', 'ucp-for-woocommerce' ),
		);
	}
	...
	public function convert_to_checkout( $cart_id, $shipping_address = null, $billing_address = null, $coupon_code = null, $customer_note = null ) {
		...
		if ( empty( $items ) ) {
			return new WP_Error(
				'cart_empty',
				__( 'Cannot checkout with an empty cart.', 'ucp-for-woocommerce' ),
				array( 'status' => 400 )
			);
		}
	...
	private function validate_product_for_cart( $product, $item ) {
		...
		if ( ! $product->is_in_stock() ) {
			return new WP_Error(
				...
				/* translators: %s: Product name */
				sprintf( __( 'Product is out of stock: %s', 'ucp-for-woocommerce' ), $product->get_name() ),
				array( 'status' => 400 )
			);
		}
	...
	private function calculate_totals( $items ) {
		...
		return array(
			...
			'note'               => __( 'Tax and shipping calculated at checkout.', 'ucp-for-woocommerce' ),
		);
	}
