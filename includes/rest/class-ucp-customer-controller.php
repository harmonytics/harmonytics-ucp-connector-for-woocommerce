...
class UCP_WC_Customer_Controller extends UCP_WC_REST_Controller {
				...
				array(
					...
					'args'                => array(
						'customer_id' => array(
							'required'          => true,
							'type'              => 'integer',
							'description'       => __( 'Customer ID.', 'ucp-for-woocommerce' ),
				...
				array(
					...
					'args'                => array(
						'email' => array(
							...
							'type'              => 'string',
							'format'            => 'email',
							'description'       => __( 'Customer email address.', 'ucp-for-woocommerce' ),
	...
	private function get_create_customer_args() {
		return array(
			...
			'first_name'       => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => __( 'Customer first name.', 'ucp-for-woocommerce' ),
				'sanitize_callback' => 'sanitize_text_field',
			),
			'last_name'        => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => __( 'Customer last name.', 'ucp-for-woocommerce' ),
				'sanitize_callback' => 'sanitize_text_field',
			),
			'username'         => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => __( 'Customer username.', 'ucp-for-woocommerce' ),
				'sanitize_callback' => 'sanitize_user',
			),
			'password'         => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => __( 'Customer password.', 'ucp-for-woocommerce' ),
			),
			'billing_address'  => array(
				'required'    => false,
				'type'        => 'object',
				'description' => __( 'Customer billing address.', 'ucp-for-woocommerce' ),
			),
			'shipping_address' => array(
				'required'    => false,
				'type'        => 'object',
				'description' => __( 'Customer shipping address.', 'ucp-for-woocommerce' ),
			),
		);
	}
	...
	private function get_add_address_args() {
		return array(
			...
			'type'        => array(
				...
				'default'     => 'shipping',
				'description' => __( 'Address type.', 'ucp-for-woocommerce' ),
			),
			'label'       => array(
				'required'          => false,
				'type'              => 'string',
				'description'       => __( 'Address label (e.g., "Home", "Work").', 'ucp-for-woocommerce' ),
				'sanitize_callback' => 'sanitize_text_field',
			),
			'set_default' => array(
				'required'    => false,
				'type'        => 'boolean',
				'default'     => false,
				'description' => __( 'Set as default address for this type.', 'ucp-for-woocommerce' ),
			),
			'address'     => array(
				'required'    => true,
				'type'        => 'object',
				'description' => __( 'Address details.', 'ucp-for-woocommerce' ),
			),
		);
	}
	...
	public function check_customer_permission( $request ) {
		...
		if ( 0 === $current_user_id && ! $this->is_authenticated() ) {
			return new WP_Error(
				'ucp_unauthorized',
				__( 'Authentication is required to access customer data.', 'ucp-for-woocommerce' ),
				array( 'status' => 401 )
			);
		}
		...
		if ( $current_user_id !== (int) $customer_id ) {
			return new WP_Error(
				'ucp_forbidden',
				__( 'You do not have permission to access this customer data.', 'ucp-for-woocommerce' ),
				array( 'status' => 403 )
			);
		}
	...
	public function check_admin_permission( $request ) {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			return new WP_Error(
				'ucp_forbidden',
				__( 'Admin or API key authentication is required for customer lookup.', 'ucp-for-woocommerce' ),
				array( 'status' => 403 )
			);
		}
	...
	public function create_customer( $request ) {
		...
		if ( $existing_user ) {
			return $this->error_response(
				'ucp_customer_exists',
				__( 'A customer with this email address already exists.', 'ucp-for-woocommerce' ),
				409
			);
		}
		...
		try {
			...
			return $this->success_response(
				array(
					...
					'message'  => __( 'Customer created successfully.', 'ucp-for-woocommerce' ),
				),
				201
			);
		...
		try {
			...
			if ( 0 === $customer->get_id() ) {
				return $this->error_response(
					...
					__( 'Customer not found.', 'ucp-for-woocommerce' ),
					404
				);
			}
		...
		try {
			...
			if ( ! empty( $data['email'] ) ) {
				...
				if ( $existing_user && $existing_user->ID !== $customer_id ) {
					return $this->error_response(
						'ucp_email_exists',
						__( 'This email address is already in use by another customer.', 'ucp-for-woocommerce' ),
						409
					);
				}
				$customer->set_email( $data['email'] );
			...
			return $this->success_response(
				array(
					'customer' => $this->mapper->map_customer( $customer ),
					'message'  => __( 'Customer updated successfully.', 'ucp-for-woocommerce' ),
				)
			);
		...
		try {
			...
			return $this->success_response(
				array(
					'customer_id' => $customer_id,
					'addresses'   => $addresses,
					'message'     => __( 'Address added successfully.', 'ucp-for-woocommerce' ),
				),
				201
			);
		...
		if ( ! $user ) {
			return $this->success_response(
				array(
					...
					'email'    => $email,
					'customer' => null,
					'message'  => __( 'No customer found with this email address.', 'ucp-for-woocommerce' ),
				)
			);
		}
