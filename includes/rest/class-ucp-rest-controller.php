...
abstract class UCP_WC_REST_Controller extends WP_REST_Controller {
	...
	/**
	 * Permission callback for intentionally public read endpoints.
	 *
	 * This method is used for endpoints that expose PUBLIC data accessible
	 * without authentication, following WordPress REST API best practices:
	 *
	 ...
	 *
	 * Per WordPress plugin guidelines, we explicitly document this is intentional.
	 * The method validates UCP is enabled before allowing access.
	 *
	 * @see https://developer.wordpress.org/reference/functions/register_rest_route/
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return bool|WP_Error True if UCP is enabled, WP_Error if disabled.
	 */
	...
	/**
	 * Permission callback for read endpoints requiring authentication.
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return bool|WP_Error
	 */
	public function check_authenticated_read( $request ) {
		if ( ! $this->is_ucp_enabled() ) {
			return new WP_Error(
				'ucp_disabled',
				__( 'UCP is currently disabled for this store.', 'harmonytics-ucp-connector-for-woocommerce' ),
		...
		return new WP_Error(
			'ucp_unauthorized',
			__( 'Authentication with read permission is required.', 'harmonytics-ucp-connector-for-woocommerce' ),
			array( 'status' => 401 )
		);
	}

	/**
	 * Permission callback for write endpoints requiring authentication.
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return bool|WP_Error
	 */
	public function check_authenticated_write( $request ) {
		if ( ! $this->is_ucp_enabled() ) {
			return new WP_Error(
				'ucp_disabled',
				__( 'UCP is currently disabled for this store.', 'harmonytics-ucp-connector-for-woocommerce' ),
		...
		return new WP_Error(
			'ucp_unauthorized',
			__( 'Authentication with write permission is required.', 'harmonytics-ucp-connector-for-woocommerce' ),
			array( 'status' => 401 )
		);
	}
	...
	public function check_admin_permission( $request ) {
		...
		return new WP_Error(
			'ucp_forbidden',
			__( 'Admin permission is required.', 'harmonytics-ucp-connector-for-woocommerce' ),
			array( 'status' => 403 )
		);
	}
