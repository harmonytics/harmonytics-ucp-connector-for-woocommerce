...
class UCP_WC_Address_Mapper {
    ...
    public function validate( $address ) {
        ...
        $required_fields = array(
            'first_name' => __( 'First name', 'ucp-for-woocommerce' ),
            'last_name'  => __( 'Last name', 'ucp-for-woocommerce' ),
            'address_1'  => __( 'Address', 'ucp-for-woocommerce' ),
            'city'       => __( 'City', 'ucp-for-woocommerce' ),
            'postcode'   => __( 'Postcode', 'ucp-for-woocommerce' ),
            'country'    => __( 'Country', 'ucp-for-woocommerce' ),
        );
        ...
        foreach ( $required_fields as $field => $label ) {
            ...
            if ( empty( $value ) ) {
                $errors[] = sprintf(
                    ...
                    __( '%s is required.', 'ucp-for-woocommerce' ),
                    $label
                );
            }
        }
        ...
        if ( ! empty( $address['country'] ) ) {
            ...
            if ( ! isset( $countries[ $address['country'] ] ) ) {
                $errors[] = __( 'Invalid country code.', 'ucp-for-woocommerce' );
            }
        }

        // Validate email if present
        if ( ! empty( $address['email'] ) && ! is_email( $address['email'] ) ) {
            $errors[] = __( 'Invalid email address.', 'ucp-for-woocommerce' );
        }
